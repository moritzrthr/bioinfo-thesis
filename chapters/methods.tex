...
\begin{figure}[H][htbp]
    ...
    \vspace{1em} % Ein wenig Abstand zwischen den Bildern
...
The genomic data, provided in annotated JSON format, was processed using the Nirvana clinical variant annotator (v3.2.3) based on the GRCh37 (hg19) genome assembly. To identify potential tumor-specific peptides and single amino acid variants (SAAVs), somatic mutations were filtered using the Variant Effect Predictor (VEP) \cite{McLaren2016}. We prioritized variants with \texttt{MODERATE} or \texttt{HIGH} functional impact, specifically missense mutations and in-frame indels, as these are most likely to result in detectable changes in the peptide sequence.
