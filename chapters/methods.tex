...
\item \textbf{K[+229.163]}: Lysine with TMT10 label.

\item \textbf{[+229.163]-}: TMT10 label at the peptide N-terminus.
...
Note:  The TMT + modification states are chemically paradoxical, as both the TMT reagent and the acetylation/ubiquination/methylation process compete for the same primary amine ($\epsilon$-amino group) of the lysine side chain.The appearance of this specific mass delta in the training data is highly unusual and occurred only in very rare instances, these occurrences require further investigation (maybe an artifact). )
...
In the baseline transformer architecture (e.g., Casanovo [Yilmaz2022]), the precursor embedding ($\mathbf{prec\_emb}$) encodes the precursor ion's mass-to-charge ratio ($m/z$) and charge state ($z$) using sinusoidal positional encodings passed through dedicated linear projection layers. 
...
The primary dataset for adapting the model to isobaric labeling is derived from the PROSPECT-MultiPTM collection \cite{Zeng2024}. These data are fundamentally based on the ProteomeTools project \cite{Zolg2017}, a large-scale effort led by the research groups of Bernhard Küster and Mathias Wilhelm to create a comprehensive synthetic human proteome. 

The utilization of these synthetic reference spectra is crucial for the de novo sequencing of TMT-labeled peptides for several reasons:

\begin{itemize}
    \item \textbf{Ground Truth for PTMs:} As the ProteomeTools library includes a vast array of synthetic peptides with predefined modifications, it provides the high-fidelity training data necessary for the Transformer model to accurately distinguish between diagnostic fragment ions and background noise.
    \item \textbf{Predictable Fragmentation:} The work by Zolg, Wilhelm, and colleagues demonstrated that synthetic peptides allow for the precise modeling of fragmentation intensities. This is particularly relevant when dealing with the increased precursor mass and the altered fragmentation patterns introduced by the TMT-labeling.
    \item \textbf{Multiplexing Adaptation:} By leveraging the PROSPECT-MultiPTM refined subset, the model can be fine-tuned to account for specific mass shifts and reporter ion interference typical for TMT-multiplexed datasets. This ensures that the de novo predictions remain robust even in complex co-isolation scenarios.
\end{itemize}
...
To prevent catastrophic forgetting during the fine-tuning process, a diverse reference dataset of unlabeled (non-TMT) spectra is included. This "Replay Set" consists of a mixture of 80\% MultiPTM data and 20\% data from the MassIVE Knowledge Base (MassIVE-KB) \cite{MassiveKB}.
...
While transformer-based architectures can theoretically also learn to consider amino acid context when mass shifts overlap, this still remains unevaluated and therefore we resolve the ambiguities. 
...
The genomic data, provided in annotated JSON format, was processed using the Nirvana clinical variant annotator (v3.2.3) \cite{Stromberg2019} based on the GRCh37 (hg19) genome assembly.  To identify potential tumor-specific peptides and single amino acid variants (SAAVs), somatic mutations were filtered using the Variant Effect Predictor (VEP) \cite{McLaren2016}. 
...
To ensure the reliability of the identified p-sites, we examined the evidence supporting each site taking into account the following factors:
...
\begin{itemize}
    ...
    \item \textbf{Cross-Platform Consistency:} Identification of "consensus sites" predicted by both MaxQuant and Modanovo-TMT.
